...
pub struct TrustChain {
    ...
    pub trust_chain: Vec<String>, // JWT strings
...
impl TrustChain {
    ...
    pub fn expiration_timestamp(&self) -> FederationResult<i64> {
        ...
        for jwt_string in &self.trust_chain {
...
impl TrustChain {
    ...
    pub fn try_new(trust_chain: Vec<String>) -> FederationResult<Self> {
        ...
        let leaf_config: EntityConfiguration = jwt_processor.extract_claims_unverified(leaf_jwt)?;
        ...
        for jwt_string in trust_chain.iter().skip(1).take(chain_len - 2) {
            let statement: EntityStatement = jwt_processor.extract_claims_unverified(jwt_string)?;
            statement.validate()?;
        ...
        let anchor_config: EntityConfiguration = jwt_processor.extract_claims_unverified(anchor_jwt)?;
    ...
    pub fn statement_at(&self, index: usize) -> FederationResult<Option<ValidatedEntityStatement>> {
        let Some(jwt) = self.trust_chain.get(index) else {
            return Ok(None);
        };

        let statement: ValidatedEntityStatement = JwtProcessor::new().extract_claims_unverified(jwt)?;
        Ok(Some(statement))
    }
    ...
    pub fn leaf_entity_id_and_configuration(&self) -> FederationResult<(EntityId, EntityConfiguration)> {
        let Some(jwt) = self.trust_chain.first() else {
            return Err(FederationError::TrustChainValidation(
                "Cannot retrieve leaf entity from empty trust chain".to_string(),
            ));
        };

        let config: EntityConfiguration = JwtProcessor::new().extract_claims_unverified(jwt)?;
    ...
    pub fn trust_anchor_entity_id_and_configuration(&self) -> FederationResult<(EntityId, EntityConfiguration)> {
        let Some(jwt) = self.trust_chain.last() else {
            return Err(FederationError::TrustChainValidation(
    ...
    pub fn intermediate_statements(&self) -> FederationResult<Vec<EntityStatement>> {
        if self.trust_chain.len() <= 2 {
            return Ok(Vec::new());
        }

        let jwt_processor = JwtProcessor::new();
        let mut statements = Vec::with_capacity(self.trust_chain.len().saturating_sub(2));
        for jwt in self.trust_chain.iter().skip(1).take(self.trust_chain.len() - 2) {
            let statement: EntityStatement = jwt_processor.extract_claims_unverified(jwt)?;
...
mod tests {
    ...
    async fn resolve_trust_chain_with_two_intermediates() {
        ...
        assert_eq!(
            validated_trustchain.trust_chain.len(),
            5,
            "Chain must have 5 statements (leaf config + 3 subordinate statements + anchor config)"
        );
        let leaf = extract_claims_unverified::<EntityConfiguration>(&validated_trustchain.trust_chain[0]).unwrap();
        assert_eq!(leaf.claims.sub, leaf.claims.iss);
        for statement in validated_trustchain.trust_chain.iter().take(4).skip(1) {
        ...
        let anchor = extract_claims_unverified::<EntityConfiguration>(&validated_trustchain.trust_chain[4]).unwrap();
        assert_eq!(anchor.claims.sub, anchor.claims.iss);
        for statement in validated_trustchain.trust_chain.iter() {
    ...
    async fn resolve_trust_chain_with_leaf_and_trust_anchor_only() {
        ...
        let leaf = extract_claims_unverified::<EntityConfiguration>(&validated_trustchain.trust_chain[0]).unwrap();
        assert_eq!(leaf.claims.sub, leaf.claims.iss);
        let statement = extract_claims_unverified::<EntityStatement>(&validated_trustchain.trust_chain[1]).unwrap();
        assert_ne!(statement.claims.sub, statement.claims.iss);
        let anchor = extract_claims_unverified::<EntityConfiguration>(&validated_trustchain.trust_chain[2]).unwrap();
        assert_eq!(anchor.claims.sub, anchor.claims.iss);
        for statement in validated_trustchain.trust_chain.iter() {
            let statement = extract_claims_unverified::<ValidatedEntityStatement>(statement).unwrap();
            match statement {
